...
case class CarbonCreateTableLikeCommand(
  ...
  override def processMetadata(sparkSession: SparkSession): Seq[Row] = {
    ...
    val srcTableInfo = srcTable.getTableInfo
    ...
    val dstTableSchema = try {
      ...
      val dis = new DataInputStream(new ByteArrayInputStream(bos.toByteArray))
      val tableSchema = new TableSchema
      tableSchema.readFields(dis)
      tableSchema
    } catch {
      case e: IOException =>
        throw new RuntimeException("Error occur while copying table schema", e)
    }
